...
void main()
{
    ...
    if(gouraud){
        ...
        vec3 reflectDir2 = reflect(-lightVector2, transformedNormal2);
        vec3 viewDir = normalize(-transformedVertex);
        vec3 viewDir2 = normalize(-transformedVertex2);
        float specAngle = max(dot(reflectDir, viewDir), 0.0);
        float specAngle2 = max(dot(reflectDir2, viewDir2), 0.0);
        float specular = pow(specAngle, mat.shinyness);
        float specular2 = pow(specAngle2, mat.shinyness);
        //Set the final color
        outColor = mat.ambientColor +
        +mat.diffuseColor*diffuse
        +mat.diffuseColor* diffuse2
        +(mat.specularColor*specular*attenuation)
        +(mat.specularColor*specular2*attenuation2);
    }
    else{
        model = mMatrix;
        view = vMatrix;
