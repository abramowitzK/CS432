...
Mesh BezierPointReader::TesselateBezierPatch(int sampling) {
    float delta = (float) (1.0 / (float)sampling);
    vec4 samples[sampling][sampling];
    //Get all the points that we need
    for(int i = 0; i < sampling; i++){
        for(int j = 0; j < sampling; j++){
            samples[i][j] = GetPointOnBezierSurface(delta*j, delta*i);
        }
    }
    ...
    unsigned index=0;
    //Now we need to generate a list of vertices for vbo...
    for(int i = 0; i < sampling-1; i++){
        for(int j = 0; j < sampling-1; j++){
            vertices.push_back(samples[i][j]);
            vertices.push_back(samples[i+1][j]);
            vertices.push_back(samples[i][j+1]);
            ...
            indices.push_back(index++);
            indices.push_back(index++);
            vertices.push_back(samples[i+1][j+1]);
            indices.push_back(index++);
            for(int k = 0; k < 4; k++){
