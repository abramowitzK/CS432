...
init( void )
{
    ...
    shapes.push_back(new Ellipsoid(vec4(0.7f, 0.0f, 0.7f, 1.0f), vec2(-0.3f, 0.0f), 0.5f, true, 0.4));
    shapes.push_back(new RegularPolygon(10, vec4(0.5f, 0.0f, 0.5f, 1.0f),vec2(-0.75f, -0.75f), 0.25f));
    shapes.push_back(new Ellipsoid(vec4(0.1f, 0.7f, 0.1f, 1.0f), vec2(0.5f, -0.3f), 0.5f, true, 1.4));
    for(Shape * s : shapes)
    {
        s->Init();
    }
